  ...
  return (
    <div className="bg-gray-200 flex">
      <div className="flex-initial">
        <SideBar
          onSelectAction={onActionChange}
          actionsList={!loading ? props.actionsList : null}
      ...
      <div className="flex-grow">
        ...
        <div className="bg-customwhite-500 flex flex-col justify-between px-40 py-20">
          ...
          <Config
            runAction={runAction}
            uploaded={UPLOADED}
            configList={props.configsObject}
          />
...
function runAction(event: any) {
  let output = JSON.parse(
    JSON.stringify(
      backend.actions.filter((action) => action.name === event[0])[0]
    )
  );
